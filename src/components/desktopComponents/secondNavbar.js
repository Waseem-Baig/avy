...
const SecondNavbar = ({ label }) => {
  const router = useRouter();
  const [pageRoute, setPageRoute] = useState("");

  useEffect(() => {
    // Map the label to a corresponding route
    const routeMapping = {
      Products: "/products",
      Dashboard: "/dashboard",
    ...
    setPageRoute(routeMapping[label] || "/dashboard");
  }, [label]);
  ...
  return (
    <div className="flex justify-center items-center w-full pt-1 px-4">
      <div className="w-[90%] relative shadow-[-10px_-10px_10px_#fff,_10px_10px_10px_rgba(174,_174,_192,_0.4)] rounded-xl bg-[#EEF5FC] border-#D4E5F6 border-[1px] border-solid box-border h-[8vh] font-source-sans-pro flex justify-between items-center">
